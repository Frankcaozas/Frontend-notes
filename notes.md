...
-   应用层
    -   提供特定应用程序的功能和服务，如电子邮件、文件传输、网页浏览等。
    -   与用户交互，并通过网络传输数据。

-   传输层
    -   提供端到端的数据传输和可靠性保证。主要负责将数据分段，并对每个分段进行编号和重新组装。
    -   为应用层提供可靠的数据传输服务，确保数据的有序性、完整性和可靠性。

-   网络层
    -   负责将数据包从源节点传输到目标节点，通过不同的网络设备实现数据包的路由选择和转发。
    -   管理不同设备之间的逻辑连接，并进行数据分组、寻址和路由。

-   数据链路层
    -   负责将比特流组装成帧，并识别物理介质上的节点之间的传输错误。
    -   提供可靠的有线或无线数据传输，通过控制物理介质的访问。

-   物理层
    -   负责传输比特流，通过物理介质（如光纤、电缆等）传输数据。
    -   主要关注一些与硬件相关的细节，如数据传输速率、电压等。
...
- 标志位（Flags）：共6个，即URG、ACK、PSH、RST、SYN、FIN等。具体含义如下：
 ...
 - SYN：发起一个新连接。
 - FIN：释放一个连接。
### ![TCP三次握手.drawio](https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost4/%E7%BD%91%E7%BB%9C/TCP%E4%B8%89%E6%AC%A1%E6%8F%A1%E6%89%8B.drawio.png)三次握手
...
- 连接：`HTTP 1.0`默认使用非持久连接，`HTTP 1.1`则默认使用持久连接。`HTTP 1.1`通过使用持久连接来使多个HTTP请求复用同一个`TCP`连接，避免了`HTTP 1.0`中使用非持久连接造成的每次请求都需要建立连接的时延。

-   缓存：`HTTP 1.0`主要使用`header`中的`If-Modified-Since`，`Expires` 来做为缓存判断的标准；`HTTP 1.1`则引入了更多的缓存控制策略，例如：`Etag`、`If-Unmodified-Since`、`If-Match`、`If-None-Match`等更多可供选择的缓存头来控制缓存策略。

-   资源请求：`HTTP 1.0`中，存在一些浪费带宽的现象，例如客户端只是需要某个对象的一部分，而服务器却将整个对象送过来了，并且不支持断点续传功能；`HTTP 1.1`则在请求头引入了`range` 头域，它允许只请求资源的某个部分，即返回码是 `206（Partial Content）`，这样就方便了开发者自由的选择以便于充分利用带宽和连接。

-   host：`HTTP 1.1`引入了`host`，用来指定服务器的域名。

-   方法：`HTTP 1.1`相较于`HTTP 1.0`新增了许多方法，如：`put`、`delete`、`options`等。
...
React18之后`setState()`都是批量更新(`batching`)
...
2. **更容易复用逻辑**: 可以通过自定义 Hook 将一些常用的逻辑封装起来，使其在不同组件中复用。这样可以提高代码的重用性和可维护性。
3. **无需理解 this**: 在 class 组件中，需要注意函数绑定、显式绑定或箭头函数来处理 this 的问题。而在函数组件中使用 Hook，则无需关心 this 的指向问题。
4. **更好的性能优化**: Hook 提供了一些性能优化的手段，例如使用 `useMemo` 和 `useCallback` 缓存计算结果，避免不必要的重渲染。
5. **更易于测试**: 函数组件相比于 class 组件更易于编写测试。使用 Hook 可以更方便地编写单元测试，以验证组件的行为和状态。
